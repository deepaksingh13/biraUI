let fieldMap = new Map([
    ...
    ['Need_replacement_of_any_item','Need replacement of any item ?']
]);
...
const creatEquipmentCondition = () =>{
    let field = ['Equipment_in_working_condition','Tower_visibility_condition','All_Assets_in_Good_Condition',
                'Is_Bira_91_Tap_Handle_in_good_condition','Is_Medallion_in_good_condition','Beer_pouring_happening_properly_or_not',
            'LED_Sign_Tower_being_lit','Machine_Exterior_clean_or_not','Beer_cold_or_not','Brand_Freshness_and_Taste','Leakage','Need_replacement_of_any_item'];
    let tmp = '';
    $('#equipCondition').html('');
    ...
    for(let i of field){
        if(i === 'Equipment_in_working_condition' || i === 'Tower_visibility_condition' 
            ...
            || i === 'Machine_Exterior_clean_or_not'){
                ...
                `;
            }else if(i === 'Equipment_in_Original_position' || i === 'Leakage' || i === 'Need_replacement_of_any_item'){
                tmp +=`
                    <div class="row">
                        <div class="col-xs-5"><label>${fieldMap.get(i)}</label></div>
